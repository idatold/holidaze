...
export default function VenueSections({ venue }) {
  ...
  const desc = (venue?.description || "").trim() || "No description provided for this venue.";
  const maxGuests = typeof venue?.maxGuests === "number" ? venue.maxGuests : null;
  const bookingsCount = Array.isArray(venue?.bookings) ? venue.bookings.length : 0;

  const amenities = [
    venue?.meta?.breakfast && { label: "Breakfast included", icon: breakfastIcon },
  ...
  const mapSrc =
    lat && lng
      ? `https://www.google.com/maps?q=${encodeURIComponent(`${lat},${lng}`)}&z=12&output=embed`
      : place
      ? `https://www.google.com/maps?q=${encodeURIComponent(place)}&z=10&output=embed`
      : null;

  return (
    <div className="mt-4">
      {/* Information (starts closed) */}
      <CollapseSection title="Information" defaultOpen={false} persistKey="venue-sec-info">
        <div className="space-y-3 text-sm">
          <p className="font-semibold text-[#006492]">{name}</p>
          <ul className="space-y-2">
            <InfoItem icon={<PeopleIcon />} label={`Max capacity: ${maxGuests ?? 0} ${maxGuests === 1 ? "guest" : "people"}`} />
            <InfoItem icon={<BookingsIcon />} label={`Previous bookings: ${bookingsCount}`} />
          </ul>
        </div>
      </CollapseSection>

      {/* Description */}
      <CollapseSection title="Description" defaultOpen={false} persistKey="venue-sec-desc">
        <p className="text-sm text-zinc-700 leading-relaxed">{desc}</p>
      </CollapseSection>

      {/* Amenities */}
      <CollapseSection title="Amenities" defaultOpen={false} persistKey="venue-sec-amen">
        {amenities.length ? (
      ...
      {/* Location */}
      <CollapseSection title="Location" defaultOpen={false} persistKey="venue-sec-loc">
        {place ? (
...
function PeopleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-5 w-5 text-[#006492]" aria-hidden="true">
      <path d="M12 12c2.2 0 4-1.8 4-4s-1.8-4-4-4-4 1.8-4 4 1.8 4 4 4z" fill="currentColor" opacity="0.3" />
      <path d="M4 19c0-2.8 3.6-5 8-5s8 2.2 8 5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
    </svg>
  );
}
function BookingsIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-5 w-5 text-[#006492]" aria-hidden="true">
      <rect x="4" y="5" width="16" height="14" rx="2.5" ry="2.5" fill="none" stroke="currentColor" strokeWidth="1.8" />
